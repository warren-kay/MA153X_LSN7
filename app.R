...
# ---- Helper: RQ box + text area ------------------------------------------
rq_block <- function(rq_id, rq_text) {
  tagList(
    div(class = "rq-box", rq_text),
    div(
...
# ---- RQ text strings (kept here so PDF handler can reuse them) -----------
rq_text <- list(
  tab1 = c(
  ...
  tabsetPanel(
    ...
    tabPanel("5. Export Responses",
      fluidPage(
        ...
        p("Complete the research questions in Tabs 1-4, then download a PDF containing your responses and instructor sample answers."),
        wellPanel(
          fluidRow(
            column(5,
            ...
            column(3,
              ...
              downloadButton("download_pdf", "Download PDF",
                             class = "btn-primary", style = "width:100%; font-size:15px;")
            )
          )
        ),
        div(style = "color: #666; font-size: 13px; background: #fff8e1;
                     border-left: 4px solid #f0ad4e; padding: 10px 14px;
                     border-radius: 0 6px 6px 0; margin-top: 10px;",
          "Note: Sample answers are not visible anywhere in this app.",
          "They appear only in the downloaded PDF, after your typed responses.")
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  # ---- Reactive ggplot objects (shared with download handler) ------------
  ...
  output$download_pdf <- downloadHandler(
    filename = function() {
      ...
      if (nchar(nm) > 0) paste0("LSN7_", gsub("[^A-Za-z0-9]", "_", nm), ".pdf")
      else "LSN7_Responses.pdf"
    },
    content = function(file) {

      # Write wrapped text starting at (x, y); return new y
      ...
      # New page: blank plotting area with [0,1]x[0,1] coordinates
      new_page <- function() {
        plot.new()
      ...
      # Print a tab's section header; return starting y for body
      ...
      # Render one tab's Q&A: questions, responses, sample answers
      ...
      pdf(file, width = 8.5, height = 11, title = "Lesson 7 Responses")
